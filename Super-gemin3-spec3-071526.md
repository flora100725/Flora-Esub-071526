ENTERPRISE INTERACTIVE SPECIFICATION & ARCHITECTURAL BLUEPRINTTFDA AI Regulatory Agentic OS & Experiential Compliance Dashboard (v4.2-ULTIMATE)1. System Vision, Gamified Regulatory Workflow & HUD Design1.1 Document Purpose & System VisionThe TFDA AI Regulatory Agentic OS & Experiential Compliance Dashboard (v4.2-ULTIMATE) marks a departure from typical tabular enterprise software by transforming medical device compliance tracking into an interactive workspace. Designed for Regulatory Affairs (RA) directors, systems validation engineers, and global medical device companies, this framework simplifies the registration of complex hardware-software assemblies under the Taiwan Food and Drug Administration (TFDA).The platform uses the Project EVO Q10 Diagnostic Ultrasound System as its benchmark device. This includes a high-density console alongside 22 active imaging transducers (convex, linear, phased array, and specialized endocavity probes) mapped against international regulations:IEC 60601-1:2025 (Ed. 4.1/4.2): Core Electrical Safety & Essential Performance.IEC 60601-1-2: Electromagnetic Disturbances & Technical Compatibility.IEC 60601-2-37 & IEC 62359: Ultrasonic Particular Safety Profiles & Acoustic Field Indices ($MI$, $TI$).ISO 10993 Series (ISO 10993-1, -5, -10, -23): Biocompatibility profiles (Cytotoxicity, Skin Irritation, and Sensitization matrices).2025/2026 TFDA AI/ML CADe/CADx Modified Technical Guidelines: Standardized independent validation, Good Machine Learning Practice (GMLP), ROC thresholds, and Area Under the Curve (AUC) variance boundaries for integrated automated measurement software.By embedding interactive, parameter-driven visual models, real-time statistical engines, and responsive UI transitions, this application aims to replace text-heavy tracking files with a responsive compliance engineering environment.                    [ 22 Transducer Multiplex Dossier Buffers ]
                                         │
                                         ▼
                 ┌───────────────────────────────────────────────┐
                 │       Secure Multi-Tenant Server Bridge       │
                 │      (Express Proxy - VPC Encrypted Port)     │
                 └───────────────────────┬───────────────────────┘
                                         │ Secure Lazy-Loaded Hook
                                         ▼
                 ┌───────────────────────────────────────────────┐
                 │          Google Gemini API Gateway            │
                 │      (gemini-3.1-pro / 3.5-Flash Active)      │
                 └───────────────────────┬───────────────────────┘
                                         │ Tool Orchestration
                                         ▼
                 ┌───────────────────────────────────────────────┐
                 │      Autonomous Agentic OS Sandbox Loop       │
                 └──────────────┬─────────────────┬──────────────┘
                                │                 │
         ┌──────────────────────┴──────┐   ┌──────┴──────────────────────┐
         ▼                             ▼   ▼                             ▼
┌──────────────────┐  ┌──────────────────┐┌──────────────────┐  ┌──────────────────┐
│  google_search   │  │  built_in_code   ││   Interactive    │  │   D3.js Node     │
│  (Live Laws &    │  │   _execution     ││ 6-Graph Canvas   │  │ Compliance Graph │
│ Accred Grounding)│  │ (Python Engine)  ││   Dashboard      │  │    Topology      │
└──────────────────┘  └──────────────────┘└──────────────────┘  └──────────────────┘
1.2 Desktop HUD Layout GridThe interface uses a dense, low-fatigue layout matching modern dark medical command centers. It scales cleanly on high-DPI desktop viewports ($\geq 1920 \times 1080$).+---------------------------------------------------------------------------------------------------------+
| SYSTEM CONTROL TOP BAR (h-16)                                                                           |
| [⚡ SYSTEM EVO OK] TFDA Regulatory Co-Pilot | Model: gemini-3.1-pro | Theme: Cyber Orchid | Mode: 22-Probe |
+---------------------------------------------------------------------------------------------------------+
| PRIMARY INTERACTIVE VIEWPORT (h-[calc(100vh-4rem)] grid grid-cols-12 gap-4 p-4 overflow-hidden)       |
| +-------------------------------------------------------+ +-------------------------------------------+ |
| | LEFT STAGE: CONTROL PANELS (col-span-7 flex flex-col) | | RIGHT STAGE: STREAM TELEMETRY (col-span-5)| |
| | +---------------------------------------------------+ | | +---------------------------------------+ | |
| | | WORKSPACE SWITCHER: [TFDA Review] | [Agentic OS]  | | | | UPPER: 6-GRAPH SPARKLINE HUD CANVAS   | | |
| | +---------------------------------------------------+ | | | - Latency: 320ms  - Session Cost: 0.12| | |
| | | DYNAMIC ACTIVE WORKSPACE COMPONENT PANEL          | | | - Entropy: Stable - Task Success: 99.7% | | |
| | |                                                   | | | +---------------------------------------+ | |
| | | * Visual Tab A: 22-Probe Dynamic Matrix           | | | | CENTER: D3.js COMPLIANCE TOPOLOGY MAP | | |
| | | * Visual Tab B: Smart RAI Deficiency Editor       | | | | - Interactive node topology network   | | |
| | | * Visual Tab C: Drag-and-Drop Agile Kanban Board  | | | | - Dynamic node highlights based on risk| | |
| | | * Visual Tab D: International Liaison Engine      | | | +---------------------------------------+ | |
| | | * Visual Tab E: Predicate Substantial Equivalence | | | | LOWER: LIVE LEDGER LOG LOG CONSOLE     | | |
| | |                                                   | | | | - Real-time JSON action log terminal  | | |
| | +---------------------------------------------------+ | | +---------------------------------------+ | |
| +-------------------------------------------------------+ +-------------------------------------------+ |
+---------------------------------------------------------------------------------------------------------+
| COMPREHENSIVE REGULATORY RUNTIME TASKBAR (h-10)                                                         |
| [PULSE GREEN] Operational Status | Active Skill: bio_auditor | Session Budget Cost: $0.01428 USD         |
+---------------------------------------------------------------------------------------------------------+
1.3 Design Tokens & Typography RulesLayout Framework: Initialized using optimized container configurations: w-full max-w-full min-h-screen bg-[#060608] text-slate-100 flex flex-col font-sans antialiased overflow-hidden select-none.Theming Color Architecture (Cyber Orchid):Base Canvas: Midnight Velvet Dark (#060608) combined with Deep Obsidian (#0c0d12) workspaces.Borders: Thin, crisp separators (border border-zinc-800/60) that shift seamlessly to glowing focus loops (focus-within:border-fuchsia-500/40 shadow-lg shadow-fuchsia-500/5).Visual Highlights: Vibrant typography accents (text-fuchsia-400) balanced with soft emerald hints (text-emerald-400) for fully verified items.Font Pairing Rule: Headlines use Space Grotesk or Outfit to emphasize high data density, body paragraphs use high-legibility sans-serif (Inter) matching modern layout standards, and monospace font families (JetBrains Mono or Fira Code) are strictly reserved for raw text buffers, formulas, and system metrics tracking.2. Integrated Data State Architecture & Real-Time Sync LoopThe application uses an Offline-First Secure Session Architecture. Data modifications cascade instantly across visual layout blocks, D3 graph maps, Monte Carlo arrays, and document generators using React state nodes.                           [ User File Drop / Text Buffer Paste ]
                                             │
                                             ▼
                              ┌─────────────────────────────┐
             ...
             ▼                               ▼                               ▼
 ┌───────────────────────┐       ┌───────────────────────┐       ┌───────────────────────┐
 │   22-Probe Matrix     │       │  Monte Carlo Process  │       │   D3.js Force Link    │
 │   Reactive Grid      │       │   Timeline Model Bins │       │   Network Topology    │
 └───────────┬───────────┘       └───────────┬───────────┘       └───────────┬───────────┘
             │                               │                               │
             └───────────────────────────────┼───────────────────────────────┘
                                             │
                              ...
                              └─────────────────────────────┘
2.1 Unified State Type Definitions (src/types.ts)TypeScriptexport type SeverityTier = "High" | "Medium" | "Low";
export type TargetStatus = "OK" | "不符合" | "Pending";
export type KanbanColumn = "todo" | "progress" | "review" | "completed";

export interface ReviewItem {
  id: string;               // Unique validation key format (e.g., "P12-BIO-5")
  probeId: string;          // Target Probe Identifier (P1 to P22)
  probeModelName: string;   // Structural model naming (e.g., "EVO-PA1-5AED")
  regulatoryCategory: string;// Checked evaluation category (e.g., "IEC-60601-2-37")
  clauseTitle: string;      // Formal standard clause descriptor
  status: TargetStatus;
...
export interface SystemMetricsTelemetry {
  latencyHistory: number[];
  contextOccupancyPct: number;
  entropyFluxNats: number;
  taskSuccessPct: number;
  toolCallCount: { search: number; codeExec: number };
  estimatedCostUSD: number;
}
3. Submissions Ingestion, Multi-Modal Text Chunking & Parsing EngineDossiers enter the engine via the multi-modal drag-and-paste file container, designed to operate in an isolated browser environment.[ Unstructured Data Buffer / Technical File Drag ]
                       │
                       ▼
    [ Encrypted Client Anonymizer Pre-processor ] ──► Strip vendor internal engineering hashes
                       │
                       ▼
    [ Multiplex Text Segment Window Handler ] ──► Partition data into safe chunks (<12,000 characters)
                       │
                       ▼
    [ Secure Server Endpoint Router /api/audit ] ──► Execute Google Gemini API Pipeline
3.1 Multiplex Technical File Upload Pipeline (server.ts)TypeScriptimport express from "express";
import multer from "multer";
import pdfParse from "pdf-parse";
import { getGeminiClient } from "./geminiHelper";

const dossierIntakeMiddleware = multer({
...
assetRouter.post("/api/audit-dossier-stream", dossierIntakeMiddleware.single("dossier"), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ error: "Ingest failure. Missing target technical dossier attachment." });
  }

  try {
    const dataBuffer = req.file.buffer;
    const rawPdfText = await pdfParse(dataBuffer);
    const sanitizedTextStream = rawPdfText.text.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ""); // Strip control characters

    const textWindows = partitionDossierText(sanitizedTextStream);
...
async function processTextWindowWithAgentCore(textChunk: string): Promise<any[]> {
  // Integrates with dynamic backend LLM model array
  return [];
}
4. High-Performance Experiential & Visual Layout Components4.1 Dynamic 22-Probe Matrix (Heatmap) ComponentThe compliance heatmap uses an interactive grid framework, mapping the 22 ultrasound transducers against core evaluation metrics.TypeScriptimport React, { useMemo } from "react";
import { ReviewItem, TargetStatus } from "../types";
...
export const ExperientialComplianceMatrix: React.FC<HeatmapProps> = ({ auditItems, focusedItemId, onSelectCell }) => {
  ...
  const resolveCellColor = (status: TargetStatus, severity: string) => {
    switch (status) {
      case "OK":
        return "bg-emerald-950/30 border-emerald-500/30 text-emerald-400 hover:bg-emerald-900/40";
      case "不符合":
        return severity === "High"
          ? "bg-rose-950/60 border-rose-500/50 text-rose-400 animate-pulse font-extrabold hover:bg-rose-900/60"
          : "bg-amber-950/30 border-amber-500/30 text-amber-400 hover:bg-amber-900/40";
      default:
        return "bg-zinc-900/80 border-zinc-800 text-zinc-500 hover:bg-zinc-800/40";
    }
  };

  return (
    <div className="w-full bg-[#0c0d12] border border-zinc-800/60 rounded-xl p-4 flex flex-col gap-3 select-none shadow-xl">
      <div className="flex justify-between items-center border-b border-zinc-800/80 pb-2">
        <h4 className="text-xs font-bold font-mono tracking-widest text-fuchsia-400 uppercase">
        ...
        <div className="min-w-[760px] grid grid-cols-7 gap-1">
          ...
          {probeSuite.map(probe => (
            <React.Fragment key={probe.id}>
              <div className="text-xs font-mono font-bold text-zinc-400 flex items-center px-1 bg-zinc-900/20 rounded">
                {probe.codeName}
              </div>
              {clauses.map(clause => {
                ...
                const borderHighlight = isFocused ? "ring-2 ring-fuchsia-500 ring-offset-1 ring-offset-slate-950 scale-[1.02]" : "";
...
4.2 Spring Physics Layout Transitions Engine (Framer Motion)To maintain layout continuity when toggling between visual tabs, the viewport container uses spring-based component transitions managed via <AnimatePresence>.TypeScriptimport React from "react";
...
export const FluidWorkspaceSwitcher: React.FC<SwitcherProps> = ({ currentTab, childrenMap }) => {
  return (
    <div className="flex-1 w-full relative min-h-[520px] overflow-hidden">
      <AnimatePresence mode="wait">
        ...
        >
          {childrenMap[currentTab] || <div className="text-zinc-600 text-xs">Pane Error: Child content missing.</div>}
        </motion.div>
      </AnimatePresence>
    </div>
  );
};
5. Core Predictive Statistical Analytics EngineThe Audit Readiness Predictor analyzes outstanding technical deficiencies and calculates likely registration timelines. It executes a 5,000-trial Monte Carlo simulation on the client side, outputting reliable probability metrics instead of arbitrary static dates.$$\text{Clearance Timeline (Days)} = \sum_{i \in \text{Deficiencies}} X_i + Y_{\text{TFDA行政審查}}$$Where:$X_i$ is a random variable representing the resolution duration for an outstanding clause $i$, modeled as uniform distributions matching targeted severity indicators:$\text{High Severity (Clinical Gaps)}: X_i \sim U(15, 35)$ days$\text{Medium Severity (Technical Reports)}: X_i \sim U(8, 22)$ days$\text{Low Severity (Administrative Errors)}: X_i \sim U(3, 12)$ days$Y_{\text{TFDA行政審查}}$ is the administrative queue overhead, modeled as a baseline standard error parameter: $Y \sim U(25, 55)$ days.TypeScript// Monte Carlo Timeline Projection Pipeline
...
export function runDossierMonteCarloSimulation(
  outstandingDefects: { severity: SeverityTier }[],
...
6. 3 Premium "Wow" AI Technical EnhancementsTo expand the platform beyond static text audits, three advanced AI capabilities are integrated to handle technical validation and cross-border data alignment.Feature G: Interactive Substantial Equivalence (SE) Predicate Matchmaker MatrixPurpose: Demonstrating Substantial Equivalence (SE) to a cleared predicate device is key to a successful TFDA registration pathway. This engine compares raw transducer specifications against cleared reference parameters and dynamically updates an equivalent matching index.  Functional Mechanics:Specification Extraction: Automatically extracts operational values (transducer frequency distributions, acoustic arrays, physical casing compositions) from raw submission files.  Weighted Matching Formula: Evaluates parameter variations to generate a unified equivalence score:$$\text{SE\_Score} = \sum_{j} W_j \cdot M_j(\text{Device}_j, \text{Predicate}_j)$$Where:$W_j$ represents the assigned weight parameter for feature column $j$.$M_j$ is a step-wise scoring function measuring technical compatibility.3.  Defensive Text Generation: If a material variance is flagged, the engine auto-drafts a tailored justification arguing that the configuration is safe based on cleared device precedents.[ Ingest Technical Spec File ] ──► [ Parameter Extraction Layer ]
                                           │
                                           ▼
...
TypeScript// Proposed Feature G API Contracts
...
└──────┬───────┘      └──────┬───────┘      └──────┬───────┘
       ...
       └─────────────────────┼─────────────────────┘
              ...
              └──────────────────────────────┘
TypeScript// Proposed Feature H Protocol Runtime Configuration
...
8. Technical Print Engine Architecture (Print CSS)The application includes a media-targeted print stylesheet that formats active views into clean document layouts, removing interactive panels and dark backgrounds during PDF exports.CSS/* Printable Submission Technical Configuration Rules Stylesheet */
@media print {
...
9. Operational Verification & Multi-Tier Deployment ArchitectureTo ensure consistent performance under production workloads, the deployment configuration is checked across a three-tier testing matrix.+-----------------------------------------------------------------------------------+
